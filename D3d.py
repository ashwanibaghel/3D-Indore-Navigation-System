...
# Simulate dummy sensor data for testing
def generate_dummy_data():
    ax = np.sin(time.time())
    ay = np.cos(time.time())
    az = 9.81

    gx = 0.1 * np.sin(time.time())
    gy = 0.1 * np.cos(time.time())
    gz = 0.1 * np.sin(time.time())
...
# Madgwick filter (for dummy data simulation)
class Madgwick:
    ...
    def update(self, gx, gy, gz, ax, ay, az, mx, my, mz):
        ...
        self.q0 = np.cos(roll / 2) * np.cos(pitch / 2) * np.cos(yaw / 2) + np.sin(roll / 2) * np.sin(pitch / 2) * np.sin(yaw / 2)
        self.q1 = np.sin(roll / 2) * np.cos(pitch / 2) * np.cos(yaw / 2) - np.cos(roll / 2) * np.sin(pitch / 2) * np.sin(yaw / 2)
        self.q2 = np.cos(roll / 2) * np.sin(pitch / 2) * np.cos(yaw / 2) + np.sin(roll / 2) * np.cos(pitch / 2) * np.sin(yaw / 2)
        self.q3 = np.cos(roll / 2) * np.cos(pitch / 2) * np.sin(yaw / 2) - np.sin(roll / 2) * np.sin(pitch / 2) * np.cos(yaw / 2)
...
while True:
    ax, ay, az, gx, gy, gz, mx, my, mz = generate_dummy_data()
    ...
    ax2d.imshow(floor_plan, extent=[0, 20, 0, 20])
